...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            conn.execute("""INSERT INTO tbl_phonebook (col_fname, col_lname, col_fullname, col_phone, col_email) VALUES (?, ?, ?, ?, ?)""", (data))
            conn.commit()
...
def addToList(self):
    ...
    if ((len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0)):
        ...
        with conn:
            cursor = conn.cursor()
            # check the db for existence in full name, if so we will alert user and disregard request
        ...
        with conn:
            ...
            if count == 0 or count2 == 0:
                ...
                if response:
                    ...
                    messagebox.showinfo("Cancel request", "No changes have been m,ade to ({}).".format(var_value))
            else:
